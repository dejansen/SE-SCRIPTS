...
// Version: 1.3
//
// NEW v1.3 (Phase 2 — Display improvements):
//   P2-1: FC() number formatter — distance display cleaner (1.2km not 1234m)
//   P2-2: SaltColor shimmer — all sprites pulse gently for living display feel
//   P2-4: FormatElapsed() uptime timer — shows runtime in header
//
// FIXES v1.2 (Phase 1):
//   P1-2a: Block lists cached (SCAN_INTERVAL ticks)
//   P1-2b: GetCargoFill cargo containers only
//   P1-2c: Drills/tanks/batteries lists cached
//   P1-2d: Speed limit restore uses _originalSpeedLimit not hardcoded 100
// ============================================================
...
public void Main(string argument, UpdateType updateSource)
{
    ...
    if (_remote == null)
    {
        ...
        {
            _remote = list[0];
            _originalSpeedLimit = _remote.SpeedLimit;
        }
    }
    ...
    if (_booting)
    {
        ...
        if (_bootTimer >= BOOT_TIME)
        {
            ...
            Echo("R.O.S Fleet Broadcast v1.3 — Online");
        }
        return;
    }
    ...
    if (_antenna == null)
    {
        DrawErrorScreen("No antenna found", "Build antenna + enable broadcasting");
        Echo("R.O.S Fleet Broadcast\nERROR: No antenna");
        return;
    }
    if (!_antenna.Enabled || !_antenna.IsBroadcasting)
    {
        DrawErrorScreen("Antenna offline", "Enable antenna and set broadcasting ON");
    ...
    if (_remote != null && _distToBase > 0)
    {
        ...
        {
            _remote.SpeedLimit  = _originalSpeedLimit;
            _approachModeActive = false;
        }
    }

    bool drilling = _drills.Count > 0 && _drills[0].Enabled;

    string   gridName  = Me.CubeGrid.CustomName;
    float    speed     = _controller != null ? (float)_controller.GetShipSpeed() : 0f;
    ...
    DrawStatusScreen(gridName, speed, status, drilling, cargoFill, fuelLevel, battLevel);

    Echo("R.O.S — Fleet Broadcast  v1.3");
    ...
    if (_distToBase > 0) Echo("Base:    " + FC(_distToBase));
    if (_approachModeActive) Echo(">> APPROACH MODE " + APPROACH_SPD + "m/s <<");
}
...
void ReScanBlocks()
{
    _drills.Clear(); _cargoCont.Clear(); _h2Tanks.Clear(); _batteries.Clear();

    GridTerminalSystem.GetBlocksOfType(_drills, d => d.IsSameConstructAs(Me));
    GridTerminalSystem.GetBlocksOfType(_cargoCont, c => c.IsSameConstructAs(Me));
    GridTerminalSystem.GetBlocksOfType(_h2Tanks, t =>
        t.IsSameConstructAs(Me) &&
        t.BlockDefinition.SubtypeId.ToLower().Contains("hydro"));
    GridTerminalSystem.GetBlocksOfType(_batteries, b => b.IsSameConstructAs(Me));

    if (_remote == null)
    {
        var remList = new List<IMyRemoteControl>();
        GridTerminalSystem.GetBlocksOfType(remList, r => r.IsSameConstructAs(Me));
        if (remList.Count > 0)
        {
            _remote = remList[0];
            _originalSpeedLimit = _remote.SpeedLimit;
...
void DrawBootScreen(float progress)
{
    ...
    {
        float cx = vp.Center.X, cy = vp.Center.Y, w = vp.Width;
        ...
        frame.Add(new MySprite { Type = SpriteType.TEXT, Data = "R.O.S",
            Position = new Vector2(cx, cy - 60f), RotationOrScale = 1.4f,
            Color = COLOR_HEADER, Alignment = TextAlignment.CENTER, FontId = "Monospace" });
        frame.Add(new MySprite { Type = SpriteType.TEXT, Data = "FLEET BROADCAST",
            Position = new Vector2(cx, cy - 32f), RotationOrScale = 0.42f,
            Color = COLOR_AMBER * 0.8f, Alignment = TextAlignment.CENTER, FontId = "Monospace" });
        frame.Add(new MySprite { Type = SpriteType.TEXT, Data = "v1.3",
            Position = new Vector2(cx, cy - 16f), RotationOrScale = 0.36f,
            Color = COLOR_DIM * 2f, Alignment = TextAlignment.CENTER, FontId = "Monospace" });
        frame.Add(new MySprite { Type = SpriteType.TEXTURE, Data = "SquareSimple",
            Position = new Vector2(cx, cy), Size = new Vector2(w * 0.6f, 1f),
            Color = COLOR_AMBER * 0.4f, Alignment = TextAlignment.CENTER });
        frame.Add(new MySprite { Type = SpriteType.TEXT,
            Data = "RevGamer (Simba \"Davy\" Jones)",
            Position = new Vector2(cx, cy + 8f), RotationOrScale = 0.28f,
        ...
        frame.Add(new MySprite { Type = SpriteType.TEXT, Data = _bootMessage,
            Position = new Vector2(cx, cy + 34f), RotationOrScale = 0.34f,
            Color = COLOR_AMBER * 0.8f, Alignment = TextAlignment.CENTER, FontId = "Monospace" });

        float barX = vp.X + 20f, barY = cy + 52f, barW = w - 40f;
        frame.Add(new MySprite { Type = SpriteType.TEXTURE, Data = "SquareSimple",
        ...
        frame.Add(new MySprite { Type = SpriteType.TEXT,
            ...
            Position = new Vector2(barX + barW, barY - 8f), RotationOrScale = 0.30f,
        ...
        frame.Add(new MySprite { Type = SpriteType.TEXTURE, Data = "Screen_LoadingBar",
            Position = new Vector2(cx, cy + 70f), Size = new Vector2(18f, 18f),
            Color = COLOR_AMBER * 0.5f, Alignment = TextAlignment.CENTER,
                       ...
                       bool drilling, float cargo, float fuel, float batt)
{
    ...
    using (var frame = surface.DrawFrame())
    {
        ...
        // P2-2: SaltColor on header elements
        AddText(frame, "R.O.S", new Vector2(x, y),
            SaltColor(COLOR_HEADER, st), 0.52f, TextAlignment.LEFT);
        AddText(frame, "v1.3", new Vector2(right, y),
            SaltColor(COLOR_DIM * 2f, st), 0.34f, TextAlignment.RIGHT);

        frame.Add(new MySprite { Type = SpriteType.TEXTURE, Data = "Screen_LoadingBar",
            Position = new Vector2(right - 20f, y + 7f), Size = new Vector2(12f, 12f),
            Color = SaltColor(COLOR_AMBER * 0.7f, st),
            Alignment = TextAlignment.CENTER, RotationOrScale = _spinnerAngle });

        y += 13f;
        AddText(frame, "  FLEET BROADCAST", new Vector2(x, y),
            SaltColor(COLOR_AMBER, st), 0.36f, TextAlignment.LEFT);
        y += 11f;
        AddText(frame, "  RevGamer (Simba \"Davy\" Jones)", new Vector2(x, y),
            SaltColor(COLOR_DIM * 1.4f, st), 0.26f, TextAlignment.LEFT);
        y += 9f;

        // P2-4: Uptime on status screen
        AddText(frame, "  UP " + FormatElapsed(_totalElapsedSec), new Vector2(x, y),
            SaltColor(COLOR_DIM * 2f, st), 0.26f, TextAlignment.LEFT);
        y += 9f;

        frame.Add(new MySprite { Type = SpriteType.TEXTURE, Data = "SquareSimple",
            Position = new Vector2(x + (w - margin * 2f) / 2f, y),
            Size = new Vector2(w - margin * 2f, 1.5f),
            Color = SaltColor(COLOR_AMBER * 0.4f, st), Alignment = TextAlignment.CENTER });
        y += 5f;

        AddText(frame, gridName, new Vector2(x, y),
            SaltColor(COLOR_YELLOW, st), fs, TextAlignment.LEFT);
        y += fs * 16f + gap;
        ...
        AddText(frame, "STATUS", new Vector2(x, y),
            SaltColor(COLOR_DIM * 2f, st), fsS, TextAlignment.LEFT);
        AddText(frame, status, new Vector2(right, y),
            SaltColor(statCol, st), fsS, TextAlignment.RIGHT);
        y += fsS * 15f + gap;

        AddText(frame, "SPEED", new Vector2(x, y),
            SaltColor(COLOR_DIM * 2f, st), fsS, TextAlignment.LEFT);
        AddText(frame, (int)speed + " m/s", new Vector2(right, y),
            SaltColor(speed > 1f ? COLOR_AMBER : COLOR_DIM * 2f, st), fsS, TextAlignment.RIGHT);
        y += fsS * 15f + gap;

        AddText(frame, "DRILL", new Vector2(x, y),
            SaltColor(COLOR_DIM * 2f, st), fsS, TextAlignment.LEFT);
        AddText(frame, drilling ? "ACTIVE" : "OFF", new Vector2(right, y),
            SaltColor(drilling ? COLOR_DANGER : COLOR_DIM * 2f, st), fsS, TextAlignment.RIGHT);
        y += fsS * 15f + gap;

        frame.Add(new MySprite { Type = SpriteType.TEXTURE, Data = "SquareSimple",
            Position = new Vector2(x + (w - margin * 2f) / 2f, y),
            Size = new Vector2(w - margin * 2f, 1f),
            Color = SaltColor(COLOR_DIM, st), Alignment = TextAlignment.CENTER });
        y += gap;
        ...
        AddText(frame, "CARGO", new Vector2(x, y),
            SaltColor(COLOR_DIM * 2f, st), fsS, TextAlignment.LEFT);
        AddText(frame, (int)cargo + "%", new Vector2(right, y),
            SaltColor(cargoCol, st), fsS, TextAlignment.RIGHT);
        y += fsS * 14f + 2f;
        AddHLine(frame, new Vector2(x, y), barW, SaltColor(COLOR_DIM * 0.6f, st), barH);
        AddHLine(frame, new Vector2(x, y), barW * (cargo / 100f), SaltColor(cargoCol, st), barH);
        y += barH + gap;

        if (batt >= 0f)
        {
            Color batCol = batt < 20f ? COLOR_DANGER : batt < 50f ? COLOR_CHARGE : COLOR_OK;
            AddText(frame, "BATTERY", new Vector2(x, y),
                SaltColor(COLOR_DIM * 2f, st), fsS, TextAlignment.LEFT);
            AddText(frame, (int)batt + "%", new Vector2(right, y),
                SaltColor(batCol, st), fsS, TextAlignment.RIGHT);
            y += fsS * 14f + 2f;
            AddHLine(frame, new Vector2(x, y), barW, SaltColor(COLOR_DIM * 0.6f, st), barH);
            AddHLine(frame, new Vector2(x, y), barW * (batt / 100f), SaltColor(batCol, st), barH);
            y += barH + gap;
        }
        ...
        if (fuel >= 0f)
        {
            ...
            AddText(frame, "FUEL", new Vector2(x, y),
                SaltColor(COLOR_DIM * 2f, st), fsS, TextAlignment.LEFT);
            AddText(frame, (int)fuel + "%", new Vector2(right, y),
                SaltColor(fuelCol, st), fsS, TextAlignment.RIGHT);
            y += fsS * 14f + 2f;
            AddHLine(frame, new Vector2(x, y), barW, SaltColor(COLOR_DIM * 0.6f, st), barH);
            AddHLine(frame, new Vector2(x, y), barW * (fuel / 100f), SaltColor(fuelCol, st), barH);
            y += barH + gap;
        }
        ...
        if (_distToBase > 0)
        {
            ...
            // P2-1: FC() for distance to base
            string distStr = FC(_distToBase) + (_distToBase >= 1000 ? "" : "m");

            AddText(frame, "BASE", new Vector2(x, y),
                SaltColor(COLOR_DIM * 2f, st), fsS, TextAlignment.LEFT);
            AddText(frame, distStr, new Vector2(right, y),
                SaltColor(_approachModeActive ? COLOR_DANGER : COLOR_AMBER, st),
                fsS, TextAlignment.RIGHT);
            y += fsS * 15f + gap;

            if (_approachModeActive)
                AddText(frame, ">> APPROACH MODE " + APPROACH_SPD + "m/s <<",
                    new Vector2(vp.X + w / 2f, y),
                    SaltColor(COLOR_DANGER, st), fsS * 0.9f, TextAlignment.CENTER);
        }
    }
}
...
void DrawErrorScreen(string title, string detail)
{
    ...
    using (var frame = surface.DrawFrame())
    {
        ...
        frame.Add(new MySprite { Type = SpriteType.TEXTURE, Data = "Danger",
            Position = new Vector2(cx, cy - 30f), Size = new Vector2(40f, 40f),
            Color = COLOR_DANGER, Alignment = TextAlignment.CENTER });
        frame.Add(new MySprite { Type = SpriteType.TEXT, Data = title,
            Position = new Vector2(cx, cy + 20f), RotationOrScale = 0.44f,
            Color = COLOR_DANGER, Alignment = TextAlignment.CENTER, FontId = "Monospace" });
        frame.Add(new MySprite { Type = SpriteType.TEXT, Data = detail,
            Position = new Vector2(cx, cy + 38f), RotationOrScale = 0.32f,
            Color = COLOR_DIM * 2f, Alignment = TextAlignment.CENTER, FontId = "Monospace" });
    }
}
...
float GetFuelLevel()
{
    ...
    foreach (var tank in _h2Tanks) { total += (float)tank.FilledRatio; count++; }
...
{
    float current = 0f, max = 0f;
    foreach (var bat in _batteries) { current += bat.CurrentStoredPower; max += bat.MaxStoredPower; }
